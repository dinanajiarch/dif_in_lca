...
p4m3_func <- function(N, k, k_size, de) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P4M3 - N{N} - C{k} - {k_size} - DE{de} - Realistic Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
    	...
    	SEED = 1495;
    	NREPS = 500;
      RESULTS = p4m3_N{N}_C{k}_{k_size}_de{de}_realistic_results.csv;"
    ),
...
%C#5%
  ...
  base_dir <- here("dissertation", "complex", "p4", "m3", "realistic")
