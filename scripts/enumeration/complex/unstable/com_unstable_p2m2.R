...
p2m2_func <- function(N, k, k_size) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P2M2 - N{N} - C{k} - {k_size} - Unstable Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
   	...
   	SEED = 1495;
   	NREPS = 500;
    RESULTS = p2m2_N{N}_C{k}_{k_size}_unstable_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "complex", "p2", "m2", "unstable")
  ...
  sim_fit <- mplusModeler(
    sim_class,
    dataout = glue(here(full_dir, "p2m2.dat")),
    modelout = glue(here(full_dir, "p2m2_c{k}.inp")),
    check = TRUE,
    run = TRUE,
    hashfilename = FALSE
  )
...
clusterExport(cl, c("p2m2_func", "p2"))
clusterEvalQ(cl, {
  library(MplusAutomation)
  library(glue)
  library(here)
...
result_list <- parLapply(cl, 1:nrow(p2), function(i) {
  p2m2_func(p2$N[i], p2$k[i], p2$k_size[i])
})
