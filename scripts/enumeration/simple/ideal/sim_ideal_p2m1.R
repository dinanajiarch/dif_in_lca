...
p2m1_func <- function(N, k, k_size, ex) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P2M1 - N{N} - C{k} - {k_size} - Ideal Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u6 x1 x2 x3;
   	...
   	NREPS = 500;
    {ex}REPSAVE = all;
    {ex}SAVE= p2m1_N{N}_C{k}_{k_size}_ideal*.csv;
    RESULTS = p2m1_N{N}_C{k}_{k_size}_ideal_results.csv;"
    ),
    ...
    MODEL =
     ...
     {ex}%C#1%
  	 ...
  	 {ex}[u1$1*-2 u2$1*-2 u3$1*-2
  	 {ex}u4$1*-2 u5$1*-2 u6$1*-2];
  ...
  base_dir <- here("dissertation", "simple", "p2", "m1", "ideal")
