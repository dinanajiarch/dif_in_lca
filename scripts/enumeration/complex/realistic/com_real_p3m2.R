...
p3m2_func <- function(N, k, k_size, de) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P3M2 - N{N} - C{k} - {k_size} - DE{de} - Realistic Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
   	...
   	SEED = 1495;
   	NREPS = 500;
     RESULTS = p3m2_N{N}_C{k}_{k_size}_de{de}_realistic_results.csv;"
    ),
...
%C#5%
  ...
  base_dir <- here("dissertation", "complex", "p3", "m2", "realistic")
