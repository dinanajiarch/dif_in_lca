...
p4m2_func <- function(N, k, k_size, de) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P4M2 - N{N} - C{k} - {k_size} - DE{de} - Ideal Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u6 x1 x2 x3;
    	...
    	SEED = 1495;
    	NREPS = 500;
      RESULTS = p4m2_N{N}_C{k}_{k_size}_de{de}_ideal_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "simple", "p4", "m2", "ideal")
