...
p4m3_func <- function(N, k, k_size, de) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P4M3 - N{N} - C{k} - {k_size} - DE{de} - Unstable Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
    	...
    	SEED = 1495;
    	NREPS = 500;
      RESULTS = p4m3_N{N}_C{k}_{k_size}_de{de}_unstable_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "complex", "p4", "m3", "unstable")
  ...
  sim_fit <- mplusModeler(
    sim_class,
    dataout = glue(here(full_dir, "p4m3.dat")),
    modelout = glue(here(full_dir, "p4m3_c{k}.inp")),
    check = TRUE,
    run = TRUE,
    hashfilename = FALSE
  )
...
clusterExport(cl, c("p4m3_func", "p4"))
clusterEvalQ(cl, {
  library(MplusAutomation)
  library(glue)
  library(here)
...
result_list <- parLapply(cl, 1:nrow(p4), function(i) {
  p4m3_func(p4$N[i], p4$k[i], p4$k_size[i], p4$de[i])
})
