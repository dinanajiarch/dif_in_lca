...
p3m2_func <- function(N, k, k_size, de) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P3M2 - N{N} - C{k} - {k_size} - DE{de} - Unstable Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
   	...
   	SEED = 1495;
   	NREPS = 500;
     RESULTS = p3m2_N{N}_C{k}_{k_size}_de{de}_unstable_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "complex", "p3", "m2", "unstable")
  ...
  sim_fit <- mplusModeler(
    sim_class,
    dataout = glue(here(full_dir, "p3m2.dat")),
    modelout = glue(here(full_dir, "p3m2_c{k}.inp")),
    check = TRUE,
    run = TRUE,
    hashfilename = FALSE
  )
...
clusterExport(cl, c("p3m2_func", "p3"))
clusterEvalQ(cl, {
  library(MplusAutomation)
  library(glue)
  library(here)
...
result_list <- parLapply(cl, 1:nrow(p3), function(i) {
  p3m2_func(p3$N[i], p3$k[i], p3$k_size[i], p3$de[i])
})
