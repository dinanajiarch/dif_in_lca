...
p4m1_func <- function(N, k, k_size, de, ex) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P4M1 - N{N} - C{k} - {k_size} - DE{de} - Unstable Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
    	...
    	NREPS = 500;
      {ex}REPSAVE = all;
      {ex}SAVE= p4m1_N{N}_C{k}_{k_size}_de{de}_unstable*.csv;
      RESULTS = p4m1_N{N}_C{k}_{k_size}_de{de}_unstable_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "complex", "p4", "m1", "unstable")
