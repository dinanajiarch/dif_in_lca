...
p2m2_func <- function(N, k, k_size) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P2M2 - N{N} - C{k} - {k_size} - Realistic Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u10 x1 x2 x3;
   	...
   	SEED = 1495;
   	NREPS = 500;
    RESULTS = p2m2_N{N}_C{k}_{k_size}_realistic_results.csv;"
    ),
...
%C#5%
  ...
  base_dir <- here("dissertation", "complex", "p2", "m2", "realistic")
