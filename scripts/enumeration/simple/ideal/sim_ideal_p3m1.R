...
p3m1_func <- function(N, k, k_size, de, ex) {
  ...
  sim_class  <- mplusObject(
    TITLE = glue("P3M1 - N{N} - C{k} - {k_size} - DE{de} - Ideal Separation;"),
    ...
    MONTECARLO = glue(
      "NAMES = u1-u6 x1 x2 x3;
   	...
   	NREPS = 500;
     {ex}REPSAVE = all;
     {ex}SAVE= p3m1_N{N}_C{k}_{k_size}_de{de}_ideal*.csv;
     RESULTS = p3m1_N{N}_C{k}_{k_size}_de{de}_ideal_results.csv;"
    ),
  ...
  base_dir <- here("dissertation", "simple", "p3", "m1", "ideal")
